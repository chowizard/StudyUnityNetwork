...
namespace Chowizard.UnityNetwork.Client.Core
{
    public class CommandLineController : MonoBehaviour
    {
        ...
        private void AnalyzeCommandAutoStartMode(int argumentIndex)
        {
            ...
            if(string.Compare(argumentValue, "server") == 0)
            {
                ...
                if(Network.NetworkManager.Instance.isAtStartup == false)
                    GameSceneManager.Instance.ChangeScene(Scene.GameScene.eSceneType.GamePlay);
                else
                    Debug.LogError("NetworkManager is not startup.");
            }
            else if(string.Compare(argumentValue, "client") == 0)
            {
                Network.NetworkManager.Instance.StartByClient();
        ...
        private void AnalyzeCommandAddress(int argumentIndex)
        {
            ...
            Network.NetworkManager.Instance.address = argumentValue;
        }
        ...
        private void AnalyzeCommandPort(int argumentIndex)
        {
            ...
            Network.NetworkManager.Instance.port = System.Convert.ToUInt16(argumentValue);
        }
    }
}
